...
-- AlterTable
ALTER TABLE `User` ADD COLUMN `fieldId` INTEGER NULL,
...
CREATE TABLE `Exam` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(191) NULL,
    `questions` VARCHAR(191) NOT NULL DEFAULT '[]',
...
-- AddForeignKey
ALTER TABLE `Material` ADD CONSTRAINT `Material_examId_fkey` FOREIGN KEY (`examId`) REFERENCES `Exam`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Result` ADD CONSTRAINT `Result_materialId_fkey` FOREIGN KEY (`materialId`) REFERENCES `Material`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
